    ...
    def __init__(self, input_channel, kernel_sizes, num_filters, dilation_rates):
        ...
        self.x0 = nn.Sequential(
            nn.Conv2d(
                ...
                padding=cal_padding(kernel_sizes[0], dilation_rates[0]),
            ),
            nn.ReLU(),
        )
        self.x1 = nn.Conv2d(
            ...
            padding=cal_padding(kernel_sizes[1], dilation_rates[1]),
        )
        self.y = nn.Conv2d(
            input_channel, num_filters[1], kernel_size=1, padding=cal_padding(1, 1)
        )
        ...
        def __init__(self):
            ...
            for i in range(0, len(num_filters), 2):
                if i == 0:
                    ...
                    input_channel = num_filters[i - 1]
                layers.append(
                    ResidualConvBlock(
                        input_channel,
                        kernel_sizes[i : i + 2],
                        num_filters[i : i + 2],
                        dilation_rates[i : i + 2],
                    )
                )
            ...
            layers.append(
                nn.Conv2d(
                    ...
                    kernel_size=1,
                    stride=1,
                    padding=cal_padding(1, 1),
                    dilation=1,
                )
            )
            layers.append(
                nn.LogSoftmax(dim=1)
            )  # explicitly compute prob here b.c. later will be used in prediction
