...
class ParagraphTextRecognizer:
    ...
    def __init__(self):
        self.line_detector_model = LineDetectorModel(
            dataset_args={"load_data": False}
        )  # This function is for prediction, thus don't need the default load_data
    ...
    def _get_line_region_crops(
        ...
        prepared_image, scale_down_factor = self._prepare_image_for_line_detector_model(
            image
        )
    ...
    def _prepare_image_for_line_detector_model(
    ...
    def _prepare_image_for_line_predictor_model(self, image: np.ndarray) -> np.ndarray:
        ...
        padded_image = np.pad(
            scaled_image, pad_width=pad_width, mode="constant", constant_values=255
        )
        prepared_image = (1 - padded_image / 255).astype(
            "float32"
        )  # black background and np.float32, both to compatible with prediction model
...
def _find_line_bounding_boxes(line_segmentation: np.ndarray):
    ...
    def _find_line_bounding_boxes_in_channel(
        ...
        line_activation_image = cv2.dilate(
        ...
        bounding_cnts, _ = cv2.findContours(
            line_activation_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
...
def _resize_image_for_line_detector_model(
