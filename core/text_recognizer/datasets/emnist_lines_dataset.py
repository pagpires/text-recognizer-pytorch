...
class EmnistLinesDataset(Dataset):
    ...
    def data_filename(self):
        return (
            DATA_DIRNAME
            / f"ml_{self.max_length}_mo{self.max_overlap}_ntr{self.num_train}_nte{self.num_test}.h5"
        )
    ...
    def _generate_data(self, split):
        ...
        if split == "train":
            samples_by_char = get_samples_by_char(
                emnist.x_train, emnist.y_train_int, emnist.mapping
            )
        else:
            samples_by_char = get_samples_by_char(
                emnist.x_test, emnist.y_test_int, emnist.mapping
            )
        ...
        with h5py.File(self.data_filename, "a") as f:
            x, y = create_dataset_of_images(
                num, samples_by_char, sentence_generator, self.max_overlap
            )
...
def construct_image_from_string(
    string: str, samples_by_char: dict, max_overlap: float
) -> np.ndarray:
...
def create_dataset_of_images(N, samples_by_char, sentence_generator, max_overlap):
    ...
    sample_image = construct_image_from_string(
        sample_label, samples_by_char, 0
    )  # Note that sample_image has 0 overlap
    images = np.zeros(
        (N, sample_image.shape[0], sample_image.shape[1]), np.uint8
    )  # pylint: disable=no-member
    labels = []
    for n in range(N):
        label = None
