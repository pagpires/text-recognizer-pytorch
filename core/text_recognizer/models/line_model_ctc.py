...
class LineModelCtc(Model):
    ...
    def __init__(
        ...
        default_network_args = {
            "window_width": 14,
            "window_stride": 5,
        }  # NOTE: this is default for load_weight thus cannot be changed during train
    ...
    def fit(
        ...
        **train_args,
        ...
        optimizer = self.optimizer()(
            self.network.parameters(), lr=3e-4
        )  # RMSProp is better than Adam in this case
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(train_sequence, 0):
                ...
                output_lengths = (
                    (torch.sum(labels != blank_idx, dim=1)).to(torch.long).to(device)
                )
                labels_concat = torch.cat(
                    [labels[i, :l] for i, l in enumerate(output_lengths)]
                )
    ...
    def evaluate(self, x, y, batch_size: int = 16, verbose: bool = True) -> float:
        ...
        test_sequence = DatasetSequence(
            x, y, batch_size, format_fn=self.batch_format_fn
        )
        with torch.no_grad():
            ...
            for i, batch in enumerate(test_sequence):
                ...
                log_soft_max, batch_input_lengths = map(
                    lambda out: out.to("cpu"), self.network(batch_x)
                )
                preds_raw.append(log_soft_max.permute(1, 0, 2))
                input_lengths.append(batch_input_lengths)
                labels_raw.append(batch_y.to("cpu"))
                output_lengths = (
                    (torch.sum(batch_y != blank_idx, dim=1)).to(torch.long).cpu()
                )

                loss = self.loss()(blank=blank_idx, reduction="mean")(
                    log_soft_max, batch_y.cpu(), batch_input_lengths, output_lengths
                )
                running_loss += loss.item()
            # preds_raw: (B, T, C)
    ...
    def predict_on_image(self, image: np.ndarray) -> Tuple[str, float]:
        ...
        pred_idx = ctc_decode(
            y_pred.permute((1, 0, 2)), input_lengths, output_length
        )  # arg[0] requires (N,T,C)
